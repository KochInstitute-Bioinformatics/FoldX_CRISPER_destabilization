process RUN_BUILDMODEL {
    ...
    path "missing_position_*.log", emit: missing_position_logs, optional: true
    ...
    if ${foldx_path} --command=BuildModel \\
        ...
        if grep -q "Specified residue not found" foldx_output.log; then
            ...
            # Create a detailed log file for missing positions
            cat > "missing_position_${gene}_${mutation}.log" << EOF
Gene: ${gene}
Mutation: ${mutation}
...
Timestamp: \$(date)
FoldX_Output: \$(grep -A 2 -B 2 "Specified residue not found" foldx_output.log || echo "Error message not captured")
EOF
            ...
            # Create a detailed log file for failed mutations
            cat > "missing_position_${gene}_${mutation}.log" << EOF
Gene: ${gene}
Mutation: ${mutation}
Error: No mutations performed
Reason: FoldX could not perform the requested mutation
Status: MUTATION_FAILED
Timestamp: \$(date)
FoldX_Output: \$(grep -A 2 -B 2 "no mutations performed" foldx_output.log || echo "Error message not captured")
EOF
            exit 0
        else
        ...
        # Create a log file for command failures
        cat > "missing_position_${gene}_${mutation}.log" << EOF
Gene: ${gene}
Mutation: ${mutation}
Error: FoldX command failed
Status: COMMAND_FAILED
Timestamp: \$(date)
FoldX_Output: \$(tail -10 foldx_output.log || echo "Could not capture FoldX output")
EOF
        exit 1
    fi
